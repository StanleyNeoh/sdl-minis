...
namespace MetaP {
    ...
    struct Callbacks<F, Fs...> {
        ...
        template <
            ...
            template<typename> typename TO_FirstArgCast,
            typename T,
            typename U,
            typename... Us
        >
        bool dispatch(T&& key, U&& arg, Us&&... args) const {
            return _dispatch<0, TO_Pred, TO_FirstArgCast>(funcs, std::forward<T>(key), std::forward<U>(arg), std::forward<Us>(args)...);
        }
    ...
    private:
        template<
            ...
            typename... Ts
        >
        static decltype(auto) _call(CT&& funcs, T&& arg, Ts&&... args) {
            if constexpr (I < std::tuple_size_v<std::decay_t<CT>>) {
                ...
                if constexpr (std::is_same_v<std::decay_t<T>, FirstArgType>) {
                    ...
                    return _call<I+1>(std::forward<CT>(funcs), std::forward<T>(arg), std::forward<Ts>(args)...);
                }
            }
        }
